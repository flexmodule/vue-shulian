...
export default {
  ...
  methods: {
    ...
    confirm() {
      this.$http.post('common/sms/verify-captcha-code',{
        captcha_token: this.captchaToken,
        captcha_code: this.vCodeInput
      }).then(res=>{
        console.log(res)
        if (res.data.rc===0) {
    ...
    getCaptcha(){
      this.captchaPath = 'http://mobile.testapi2.healthydoc.com.cn/common/get-captcha?captcha_token='+this.captchaToken +'&refresh='+(new Date()).valueOf()
    },
    refreshCaptcha(){
      this.getCaptcha()
      this.vCodeInput = ''
